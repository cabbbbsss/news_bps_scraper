...
@st.cache_data
def update_database_connection_status_async(placeholder):
    """Update database connection status asynchronously."""
    try:
        # Show loading status
        with placeholder:
            st.markdown('<p style="color: #ffc107;">⏳ Checking...</p>', unsafe_allow_html=True)

        # Test connection
        conn_test = get_mysql_conn()
        if conn_test:
            conn_test.close()
            status = "connected"
            display_text = '<p class="status-connected">🟢 Connected</p>'
        else:
            status = "disconnected"
            display_text = '<p class="status-error">🔴 Disconnected</p>'

    except Exception as e:
        status = "error"
        display_text = '<p class="status-error">🔴 Error</p>'

    # Update session state
    st.session_state.db_connection_status = status
    st.session_state.db_connection_last_check = time.time()
...
def update_database_connection_status():
    """Update cached database connection status with background threading."""
    current_time = time.time()

    # Only check connection every 30 seconds to avoid slow re-renders
    if (st.session_state.db_connection_last_check is None or
        current_time - st.session_state.db_connection_last_check > 30):

        # Create placeholder for async update
        placeholder = st.empty()

        # Start background thread for connection test
        thread = threading.Thread(
            target=update_database_connection_status_async,
            args=(placeholder,),
            daemon=True
        )
        thread.start()
        ...
        st.session_state.db_status_placeholder = placeholder

    else:
        # Show cached status immediately
        display_cached_status()

def display_cached_status():
    """Display cached database connection status."""
    if st.session_state.db_connection_status is None:
        # First time loading - show loading status
        st.markdown('<p style="color: #ffc107;">⏳ Loading...</p>', unsafe_allow_html=True)
        ...
        # Add fallback scraping functionality
        st.info("💡 **Alternatif: Scraping Langsung dari Media**")
        st.markdown("Database server tidak aktif? Anda tetap bisa mengambil berita langsung dari media online.")

        with st.expander("📰 Scraping Manual (Database Offline)", expanded=False):
            ...
            with col1:
                ...
                media_sources = {
                    "GOSULUT.ID": {
                        "url": "https://gosulut.id/category/daerah/provinsi-gorontalo/",
                        "module": "scraper_gosulut",
                        "function": "fetch_articles"
                    },
                    "Gorontalo Post": {
                        "url": "https://gorontalopost.id/category/berita/",
                        "module": "scraper_gopost",
                        "function": "fetch_articles"
                    },
                    "Habari.id": {
                        "url": "https://habari.id/category/gorontalo/",
                        "module": "scraper_habari",
                        "function": "fetch_articles"
                    },
                    "Rakyat Gorontalo": {
                        "url": "https://www.rakyatgorontalo.com/",
                        "module": "scraper_rakyatgorontalo",
                        "function": "fetch_articles"
                    },
                    "GoPOS.id": {
                        "url": "https://gopos.id/category/berita/",
                        "module": "scraper_gopos",
                        "function": "fetch_articles"
                    },
                    "Antara News": {
                        "url": "https://gorontalo.antaranews.com/",
                        "module": "scraper_antara",
                        "function": "fetch_articles"
                    },
                    "Gorontalo Prov": {
                        "url": "https://gorontaloprov.go.id/category/berita/",
                        "module": "scraper_gorontaloprov",
                        "function": "fetch_articles"
                    },
                    "CoolTurnesia": {
                        "url": "https://coolturnesia.com/category/gorontalo/",
                        "module": "scraper_coolturnesia",
                        "function": "fetch_articles"
                    }
                }
                ...
                selected_media = st.selectbox(
                    "Pilih Media Sumber:",
                    list(media_sources.keys()),
                    help="Pilih media online Gorontalo yang ingin di-scrape"
                )
            ...
            with col2:
                ...
                start_date = st.date_input(
                    "Tanggal Mulai:",
                    default_start,
                    help="Artikel dari tanggal ini ke atas"
                )
                end_date = st.date_input(
                    "Tanggal Akhir:",
                    datetime.now(),
                    help="Artikel sampai tanggal ini"
                )
            ...
            with col3:
                ...
                keyword = st.text_input(
                    "Kata Kunci (Opsional):",
                    "",
                    help="Filter artikel yang mengandung kata kunci tertentu"
                )
                ...
                max_articles = st.number_input(
                    ...
                    max_value=100,
                    value=20,
                    help="Batas maksimal artikel yang akan diambil"
                )

            # Scraping execution
            if st.button("🚀 Mulai Scraping", type="primary", use_container_width=True):
                if start_date > end_date:
                    st.error("❌ Perbaiki tanggal terlebih dahulu!")
                else:
                    with st.spinner("🔍 Mengambil artikel dari web..."):
                        try:
                            # Import the appropriate scraper
                            media_config = media_sources[selected_media]
                            module_name = media_config["module"]
                            function_name = media_config["function"]
                            ...
                            # Prepare parameters
                            category_url = media_config["url"]

                            # Execute scraping (without database saving)
                            articles = scraper_func(
                                category_url=category_url,
                                start_date=start_date,
                                end_date=end_date,
                                db_config=None,  # No database saving
                                max_pages=10  # Limit pages for quick results
                            )
                            ...
                            if keyword and articles:
                                ...
                                for article in articles:
                                    content_text = f"{article.get('title', '')} {article.get('contents', '')}".lower()
                                    if keyword_lower in content_text:
                                        filtered_articles.append(article)
                                articles = filtered_articles[:max_articles]  # Limit results
                            else:
                                articles = articles[:max_articles] if articles else []

                            # Store results in session state for display
                            st.session_state.fallback_articles = articles
                            st.session_state.fallback_media = selected_media
                            st.session_state.fallback_filters = {
                                "start_date": start_date,
                                "end_date": end_date,
                                "keyword": keyword
                            ...
                            st.success(f"✅ Berhasil mengambil {len(articles)} artikel dari {selected_media}")

                        except Exception as e:
                            st.error(f"❌ Gagal melakukan scraping: {str(e)}")
                            st.session_state.fallback_articles = []

            # Display results if available
            if 'fallback_articles' in st.session_state and st.session_state.fallback_articles:
                articles = st.session_state.fallback_articles
                media = st.session_state.get('fallback_media', 'Unknown')
                filters = st.session_state.get('fallback_filters', {})

                st.markdown("---")
                st.subheader(f"📄 Hasil Scraping: {media}")

                # Show applied filters
                with st.expander("🔍 Filter yang Diterapkan", expanded=False):
                    st.write(f"**Media:** {media}")
                    st.write(f"**Tanggal:** {filters.get('start_date', 'N/A')} - {filters.get('end_date', 'N/A')}")
                    if filters.get('keyword'):
                        st.write(f"**Kata Kunci:** '{filters['keyword']}'")

                # Show article count
                st.metric("📊 Total Artikel", len(articles))

                # Display articles
                for i, article in enumerate(articles, 1):
                    with st.expander(f"📰 Artikel {i}: {article.get('title', 'No Title')[:60]}...", expanded=(i<=3)):
                        col1, col2 = st.columns([3, 1])

                        with col1:
                            st.markdown(f"**📅 Tanggal:** {article.get('date', 'N/A')}")
                            st.markdown(f"**👤 Reporter:** {article.get('reporter', 'N/A')}")
                            st.markdown(f"**🔗 Sumber:** {article.get('sources', 'N/A')}")

                        with col2:
                            # Classify BPS category for this article
                            article_text = f"{article.get('title', '')} {article.get('contents', '')}"
                            bps_category = classify_bps_category(article_text)
                            bps_detail = BPS_CATEGORIES.get(bps_category, bps_category)

                            st.markdown(f"**🏷️ BPS:** {bps_category}")
                            st.caption(bps_detail[:50] + "..." if len(bps_detail) > 50 else bps_detail)

                        st.markdown("---")
                        st.markdown("**📝 Konten:**")
                        content = article.get('contents', 'No content available')
                        st.write(content[:500] + "..." if len(content) > 500 else content)

                        if article.get('links'):
                            st.markdown(f"**🔗 Link:** [{article['links']}]({article['links']})")

                # Download option
                if articles:
                    # Convert to DataFrame for download
                    df_download = pd.DataFrame([{
                        'tanggal': art.get('date'),
                        'judul': art.get('title'),
                        'konten': art.get('contents'),
                        'reporter': art.get('reporter'),
                        'sumber': art.get('sources'),
                        'link': art.get('links'),
                        'kategori_bps': classify_bps_category(f"{art.get('title', '')} {art.get('contents', '')}")
                    } for art in articles])
                    ...
                    st.download_button(
                        label="📥 Download CSV",
                        data=csv_data,
                        file_name=f"scraping_{media}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                        mime="text/csv",
                        key="download_fallback_csv"
                    )
    else:
...
if config_path.exists():
    try:
        ...
        st.info("📄 Configuration loaded from config.ini (development mode)")
    except Exception as e:
        st.warning(f"⚠️ Failed to read config.ini: {e}. Using environment variables only.")
else:
    st.info("🌐 Configuration loaded from environment variables (production mode)")
...
db_config = {
    "host": os.getenv("DB_HOST", config.get("DEFAULT", "DB_HOST", fallback="localhost")),
    "user": os.getenv("DB_USER", config.get("DEFAULT", "DB_USER", fallback="root")),
    "password": os.getenv("DB_PASSWORD", config.get("DEFAULT", "DB_PASSWORD", fallback="")),
    "database": os.getenv("DB_NAME", config.get("DEFAULT", "DB_NAME", fallback="news_database")),
    "charset": "utf8mb4",
...
azure_config.update({
    "endpoint": os.getenv("AZURE_OPENAI_ENDPOINT", config.get("DEFAULT", "AZURE_OPENAI_ENDPOINT", fallback="")),
    "api_key": os.getenv("AZURE_OPENAI_API_KEY", config.get("DEFAULT", "AZURE_OPENAI_API_KEY", fallback="")),
    "api_version": os.getenv("AZURE_OPENAI_API_VERSION", config.get("DEFAULT", "AZURE_OPENAI_API_VERSION", fallback="2024-12-01-preview")),
    "deployment_name": os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME", config.get("DEFAULT", "AZURE_OPENAI_DEPLOYMENT_NAME", fallback="gpt-4o"))
})

# Check Azure OpenAI configuration after config is loaded
if 'AZURE_OPENAI_AVAILABLE' not in locals():
    AZURE_OPENAI_AVAILABLE = False

if LANGCHAIN_AVAILABLE:
    AZURE_OPENAI_AVAILABLE = (
        azure_config["endpoint"]
        and azure_config["api_key"]
        and azure_config["endpoint"] != "https://your-azure-openai-endpoint.openai.azure.com/"
        and azure_config["api_key"] != "your-azure-openai-api-key"
    )
...
if st.session_state.scraper_mode == "Web Scraper":
    if st.session_state.filtered_df is not None and not st.session_state.filtered_df.empty:
        ...
        with col4:
            # Calculate most common BPS category
            if 'bps_category' in df.columns and not df['bps_category'].empty:
                top_bps_category = df['bps_category'].value_counts().index[0] if not df['bps_category'].isna().all() else "UMUM"
                top_bps_name = BPS_CATEGORIES.get(top_bps_category, top_bps_category)
            else:
                top_bps_category = "UMUM"
                top_bps_name = "UMUM"
            st.metric(
                label=f"🏷️ Top BPS: {top_bps_category}",
                value=top_bps_name[:20] + "..." if len(top_bps_name) > 20 else top_bps_name,
                help=f"Most common BPS category: {top_bps_name}"
            )
            ...
            with insight_col1:
                ...
                if 'bps_category' in df.columns:
                    ...
                    if not bps_counts.empty:
                        ...
                        st.info(f"🏷️ **Top BPS:** {dominant_bps} ({dominant_name[:15]}...)")

            with insight_col2:
                # Source diversity
                if 'sources' in df.columns:
                    unique_sources = df['sources'].dropna().nunique()
                    st.info(f"📰 **Sources:** {unique_sources} unique")

            with insight_col3:
                avg_title_length = df['title'].str.len().mean() if 'title' in df.columns else 0
                st.info(f"📏 **Avg Title:** {avg_title_length:.0f} chars")
...
# Filter summary - only show in Web Scraper mode
if st.session_state.scraper_mode == "Web Scraper" and st.session_state.last_query:
    st.subheader("📋 Current Web Scraper Filters")
    config_col1, config_col2 = st.columns(2)

    with config_col1:
        st.write("**Date Range:**")
        st.write(f"{st.session_state.last_query['start_date']} to {st.session_state.last_query['end_date']}")

        st.write("**Sources:**")
        st.write("- All Gorontalo news sources")

    with config_col2:
        st.write("**Keywords:**")
        if st.session_state.last_query['keywords']:
            for kw in st.session_state.last_query['keywords']:
                st.write(f"- {kw}")
        else:
            st.write("- (none)")

        if st.session_state.query_results:
            st.write(f"**Results:** {len(st.session_state.query_results)} articles found")
